...
if(!empty($_POST)){
    $name=$_POST['name'];
    $gender=$_POST['gender']=='man' ?'男':'女';
    $birthdate=$_POST['birthdate'];
    $department=$_POST['department'];
    $remakes=$_POST['remarks'];
    $link =mysqli_connect('localhost','root','zhao/980931','learn')
    mysqli_set_charset($link,'utf8');
    $sql="insert into employee(name,gender,birhdate,department,remakrs) values ('$name','$gender','birthdate','department','remarks')";
    $reuslt=@mysqli_query($link,$sql) or exit ('员工添加失败'.mysqli_error($link));
